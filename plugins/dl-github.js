let handler = async (m, { args, usedPrefix, command }) => {
    ...
    if (!args[0]) {
        return m.reply('Username and Repository name is missing. Example: ⚢✞ D@¥ID✞⚢ 𝚀𝚄𝙴𝙴𝙽 𝙹𝙴𝚂𝚂𝙸𝙲𝙰 𝚇𝙼𝙳');
    }

    // Check if the repository (args[1]) is provided
    if (!args[1]) {
        return m.reply('Repository name is missing. Example: 𝚀𝚄𝙴𝙴𝙽 𝙹𝙴𝚂𝚂𝙸𝙲𝙰 𝚇𝙼𝙳');
    }
